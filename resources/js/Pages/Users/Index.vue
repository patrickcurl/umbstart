        ...
        <tr v-for="user in users" :key="user.id" class="hover:bg-grey-lightest focus-within:bg-grey-lightest">
          ...
          <td class="border-t">
            <div class="flex items-end float-right">
              <inertia-link v-if="permissions.impersonate === true" class="btn-indigo  px-4 py-3 pt-2 mr-2" :href="route('impersonate.start', user.id)">
                <span>Impersonate</span>
              </inertia-link>
              <inertia-link class="btn-indigo mt-2 px-4 py-3 pt-2 mr-2" :href="route('users.edit', user.id)" tabindex="-1">
                <span>Edit</span>
