<template>
  <div class="flex items-center">
    <div class="flex w-full bg-white shadow rounded">
      <dropdown class="px-2 sm:w-48 rounded-l border-r hover:bg-grey-lightest focus:border-white focus:shadow-outline focus:z-10" placement="bottom-start" :show="showDropdown">
        <div class="flex items-baseline">
          <span class="text-grey-darkest hidden md:inline">{{ taxonomy }}</span>
    ...
    <button class="ml-3 text-sm text-grey-dark hover:text-grey-darker focus:text-indigo" type="button" @click="$emit('reset')">
      Reset
    </button>
  </div>
</template>
